...
import com.garood.eureka.client.feign.clients.IStudentService;
...
public class SchoolServiceController implements ISchoolServiceController {
    ...
    //we have two way to make a servie call
    //1 is By RestTemplate, but in this case we need to provide the url
    // 2 is by FeignClient, which is the declarative way to make the service call
    //via service name and Id
    @Autowired
    RestTemplate restTemplate;

    @Autowired
    private IStudentService studentServiceFeignClient;
    ...
    public String getStudents(String schoolname) {
        ...
        String response = studentServiceFeignClient.getStudents(schoolname);
